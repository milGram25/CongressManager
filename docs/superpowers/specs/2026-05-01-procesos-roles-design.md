...
- El campo `password` solo es requerido cuando `rol == "administrador"`
- Valida la contraseña del usuario autenticado (el admin que hace el request) con `authenticate()`
...
- No requiere contraseña para quitar ningún rol
...
- Al activar un toggle → llama `assignRoleApi`
  - Si es "administrador": primero muestra campo de contraseña inline; el botón "Confirmar" ejecuta la llamada
  - Si es otro rol: ejecuta la llamada inmediatamente
- Al desactivar un toggle → llama `removeRoleApi` inmediatamente (sin confirmación extra)
...
| Caso | Comportamiento |
|---|---|
| Contraseña incorrecta al asignar admin | Backend devuelve 401, toast "Contraseña incorrecta", toggle no cambia |
...
- Confirmación de contraseña para quitar el rol de administrador
