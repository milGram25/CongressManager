-- Ejecutar este script UNA VEZ antes de correr las migraciones de Django.
-- Es idempotente: puedes correrlo más de una vez sin errores.
-- Las tablas dictaminador_congreso y evaluador_congreso las gestiona Django (migrate).
...
CREATE TABLE IF NOT EXISTS areas_generales (
...
CREATE TABLE IF NOT EXISTS costos_congreso (
...
CREATE TABLE IF NOT EXISTS fechas_congreso (
...
CREATE TABLE IF NOT EXISTS dictamen_pregunta (
...
CREATE TABLE IF NOT EXISTS congreso (
    ...
    id_rubrica_default INTEGER REFERENCES rubrica(id_rubrica),
...
DO $$ BEGIN
    ALTER TABLE tipo_trabajo ADD CONSTRAINT fk_tipo_trabajo_congreso
        FOREIGN KEY (id_congreso) REFERENCES congreso(id_congreso) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
    ALTER TABLE rubrica ADD CONSTRAINT fk_rubrica_congreso
        FOREIGN KEY (id_congreso) REFERENCES congreso(id_congreso) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE TABLE IF NOT EXISTS dictaminador (
    id_dictaminador SERIAL PRIMARY KEY,
    id_persona INTEGER NOT NULL REFERENCES persona(id_persona)
);

-- NOTA: dictaminador_congreso y evaluador_congreso son gestionadas por Django (users/0003).
-- NO las crees aquí; Django las crea automáticamente con `python manage.py migrate`.
...
CREATE TABLE IF NOT EXISTS extenso (
...
CREATE TABLE IF NOT EXISTS dictamen_resumen (
    id_dictamen SERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS ponente_has_ponencia (
...
CREATE TABLE IF NOT EXISTS constancia (
    ...
    estatus VARCHAR(20) DEFAULT 'generada',
...
CREATE TABLE IF NOT EXISTS libro_has_ponencia (
...
CREATE TABLE IF NOT EXISTS ponencia_magistral (
...
CREATE TABLE IF NOT EXISTS ponencia_magistral_has_ponente_magistral (
