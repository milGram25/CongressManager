...
export default function AgendaList({ events = [], loading = false, error = "" }) {
  ...
  return (
    <div className="max-w-3xl mx-auto w-full bg-base-100 rounded-xl shadow-sm border border-base-300 overflow-hidden">
      ...
      <div className="px-6 md:px-8 pb-8">
        {loading ? (
          <div className="py-12 text-center opacity-50 italic">Cargando agenda...</div>
        ) : error ? (
              ...
              filteredSchedule.map((item, index) => (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-xs opacity-60 block">{item.time?.split(" ").slice(1).join(" ") || ""}</span>
                  </div>
                </div>
              ))
            ) : (
