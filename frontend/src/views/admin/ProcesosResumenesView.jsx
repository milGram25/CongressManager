...
function exportToCSV(items) {
  ...
  const rows = items.map(i => [i.id, `"${i.title}"`, i.asignado ? "Sí" : "No", i.revisado ? "Sí" : "No", i.aceptado ? "Sí" : "No"]);
  ...
  return (
    <div className={`flex items-center gap-3 py-3 px-1 border-b border-base-200 last:border-0 transition-colors rounded-xl ${selected ? "bg-[#e8f5f5]" : "hover:bg-base-200/40"}`}>
      ...
      <div className="flex items-center gap-1.5 bg-base-200 rounded-full px-3 py-1.5 flex-shrink-0">
        ...
        <IconBtn active={item.asignado} title="Asignado a revisor"><MdPerson size={14} /></IconBtn>
        <StatusDot active={item.revisado} />
        <IconBtn active={item.revisado} title="Revisado"><MdPanTool size={14} /></IconBtn>
        <StatusDot active={item.aceptado} />
        <IconBtn active={item.aceptado} title="Aceptado"><MdCheck size={15} /></IconBtn>
      </div>
    </div>
  );
}
              ...
              {[
                ...
                <div key={r.label} className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-sm text-base-content/70">
                    <span className="text-[#00868a]">{r.icon}</span>{r.label}
                  </div>
