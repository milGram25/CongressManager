...
function exportToCSV(items) {
  ...
  a.href = url; a.download = "extensos.csv"; a.click();
      ...
      {item.revisado ? (
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-green-100 flex items-center justify-center">
            <MdCheck size={13} className="text-green-600" />
          </div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-6 h-6 rounded-full bg-amber-100 flex items-center justify-center">
              <MdPanTool size={12} className="text-amber-600" />
            </div>
      ...
      {item.aceptado ? (
        ...
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-base-200 flex items-center justify-center">
            <MdClose size={12} className="text-base-content/40" />
          </div>
  ...
  return (
    <div className="bg-base-100 p-6 rounded-3xl border border-base-300 shadow-sm" style={{ width: 560 }}>
      ...
      <div className="overflow-y-auto" style={{ maxHeight: 380 }}>
        {filtered.length === 0 ? (
          <div className="text-center py-10 text-sm text-base-content/40 italic">No se encontraron extensos con ese filtro.</div>
        ) : (
          filtered.map(item => (
            <ExtensoRow key={item.id} item={item} selected={selected.includes(item.id)} onSelect={toggleSelect} onView={setViewItem} />
          ))
        )}
      </div>
